section .data
txt: db "section .data%ctxt: db %c%s%c, 0%cflag: db %cw+%c, 0%cnew_file: db %cSully_%%d.s%c, 0%ccommand: db %cnasm -f macho64 Sully_%%d.s && gcc -Wall -Wextra -Werror Sully_%%d.o -o Sully_%%d && ./Sully_%%d%c, 0%ccommand2: db %cnasm -f macho64 Sully_%%d.s && gcc -Wall -Wextra -Werror Sully_%%d.o -o Sully_%%d%c, 0%c%%assign NUM  %d%cfile: db __FILE__, 0%cglobal _main%cextern _asprintf%cextern _free%cextern _fprintf%cextern _strchr%cextern _system%cextern _fopen%cextern _fclose%csection .text%c_main:%cpush rbp%cmov rbp, rsp%csub rsp, 16%cmov r15, NUM%ccmp r15, 0%cjl return%clea rdi, [rel file]%cmov rsi, '_'%ccall _strchr%ccmp rax, 0%cje open%cdec r15%copen:%clea rdi, [rel rbp - 16]%clea rsi, [rel new_file]%cmov rdx, r15%cxor rax, rax%ccall _asprintf%cmov rdi, [rbp - 16]%clea rsi, [rel flag]%ccall _fopen%ccmp rax, 0%cje return%cmov r14, rax%cmov rdi, [rbp - 16]%ccall _free%cmov rdi, r14%clea rsi, [rel txt]%cmov rdx, 10%cmov rcx, 34%clea r8, [rel txt]%cmov r9, 34%cxor rax, rax%cmov r10, 0%cnew_line:%cpush 10%ccmp r10, 107%cje print%cinc r10%cjmp new_line%cprint:%cpush r15%cpush 10%cpush 34%cpush 34%cpush 10%cpush 34%cpush 34%cpush 10%cpush 34%cpush 34%cpush 10%cpush 34%cpush 34%cpush 10%ccall _fprintf%cmov rdi, r14%ccall _fclose%ccmp r15, 0%cjg cmd%ccmp r15, 0%cje cmd2%cjmp return%ccmd:%clea rdi, [rel rbp - 16]%clea rsi, [rel command]%cmov rdx, r15%cmov rcx, r15%cmov r8, r15%cmov r9, r15%cxor rax, rax%ccall _asprintf%cmov rdi, [rbp - 16]%ccall _system%cmov rdi, [rbp - 16]%ccall _free%cjmp return%ccmd2:%clea rdi, [rel rbp - 16]%clea rsi, [rel command2]%cmov rdx, r15%cmov rcx, r15%cmov r8, r15%cxor rax, rax%ccall _asprintf%cmov rdi, [rbp - 16]%ccall _system%cmov rdi, [rbp - 16]%ccall _free%creturn:%cleave%cret%c", 0
flag: db "w+", 0
